...
1) para usar el espacion que hay abajo se podria usar una grilla tipo
   la del fechador que te muestre las cuotas y los pagos a medida que
   avanzas en el listado.
2) hay alguna razon para que no se use el dot para poner la fecha de hoy?
3) ver si el resaltado de los pagados y cancelados no solo funciona
...
4) se debe resaltar un pagado que permanezca con una fecha baja
5) forma de poner no vender mas dentro del fechador y ver si esta en no
   vender mas. para prescindir de python
* listados 
** ponerle fecha a los listados
